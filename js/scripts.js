...
function updateCounters() {
  var counters = document.querySelectorAll(".number-count");
  counters.forEach(function (counter) {
    var targetValue = parseFloat(counter.getAttribute("data-count"));
    var currentValue = parseFloat(counter.innerText);
    var increment = targetValue / 70; // 7 seconds interval, 10 intervals for smooth transition
    if (currentValue < targetValue) {
      counter.innerText = (currentValue + increment).toFixed(1);
    }
  });
}

// Set interval to update counters every 7 seconds
setInterval(updateCounters, 1000 * 7);

// Initial update to set correct values
updateCounters();

//   // Show alert on page unload/refresh
//   window.addEventListener('beforeunload', function(event) {
// 	event.preventDefault();
// 	event.returnValue = '';
// 	alert("Data can be injured if you refresh!");
//   });
...
// JavaScript to show the modal when the user refreshes the page
...
function showRefreshWarning() {
  var modal = document.getElementById("refreshWarning");
  modal.style.display = "block";

  // Hide the modal after a few seconds (adjust the timeout as needed)
  setTimeout(function () {
